...
def apply_bilateralFilter(img, sigma_s, sigma_r):
...
sigma_r = 0.1 * 255
for sigma_s in np.arange(0.0, 15.0, 0.075):

    if(sigma_s == 0):
        io.imsave(f"generatedPics/{sigma_s}.png", beforeImage)
        continue

    generatedImage = apply_bilateralFilter(beforeImage, sigma_s, sigma_r)

    io.imsave(f"generatedPics/{sigma_s}.png", generatedImage)
